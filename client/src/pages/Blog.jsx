...
const Blog = () => {
  ...
  const fetchBlogData = async ()=>{
    // <-- Here earlier we use to access the local data from assets folder i just this comment for my future understanding -->
  ...
  try {
    const { data } = await axios.get(`/api/blog/comments`, {params: { blogId: id } //this is the correct way to send query params
    });
...
// in the above hook there is no need for the dependemcie in the array
...
{/* adding Comment section */}
        <div>
          ...
          <form onSubmit={addComment} className='flex flex-col items-start gap-4 max-w-lg'>
            <input onChange={(e)=> setName(e.target.value) } value={name} className='w-full p-2 border border-gray-300 rounded outline-none
        ...
        {/* Share Button */}
        <div>
          <p>Share this article on Socil Media</p>
          <div className='flex'>

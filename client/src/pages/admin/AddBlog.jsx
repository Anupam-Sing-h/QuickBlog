...
const AddBlog = () => {
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex-1 bg-blue-50/50 text-gray-600 h-full overflow-scroll'>
      <div className='bg-white w-full max-w-3xl p-4 md:m-10 sm:m-10 shadow rounded'>
        ...
        <input className='w-full border border-gray-300 mt-2 p-2 max-w-lg outline-none rounded' type="text" placeholder='Enter sub title' value={subTitle} onChange={(e) => setSubTitle(e.target.value)} required />
         ...
         <p className='mt-4'>Blog Description</p>
          <div className='max-w-lg h-72 pb-16 sm:pb-10 pt-2 relative'>
            ...
            <button type='button' onClick={generateContent} disabled={loading} className='absolute right-2 bottom-1 ml-2 text-xs text-white bg-black/70 px-4 py-1.5 rounded hover:underline cursor-pointer'>
          ...
          <button disabled={isAdding} type='submit' className='mt-8 w-40 h-10 bg-primary text-white rounded cursor-pointer text-sm'>{isAdding ? 'Adding...' : 'Add Blog'}
          </button>        
